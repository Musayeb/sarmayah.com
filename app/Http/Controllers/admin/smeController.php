...
class smeController extends Controller
{
    ...
    public function sme_profile_update(Request $request){
     ...
     public function market_life_cycle(Request $request){
      ...
      public function busniess_modal_content($data,$hide){

        $buniess=DB::table('busniess_model_convas')->select($data)->where('entre_id',$hide)->get();        
      ...
      public function sme_market_position(Request $request){
        ...
        $ent=DB::table('market_positionning')->where('entre_id',$request->hide_input)->get();
    // echo $request->hide_input;
    //     exit;
        ...
        if($request->hasFile('market_positon_file')){
         if ($datavalide == true) {
          ...
          $path= Storage::putFile('/public/market_position',$request->file('market_positon_file'));
            DB::table('market_positionning')->where('entre_id',$request->hide_input)
    ...
    public function b_head_summary(Request $request){
        $data = json_decode($request->inputdata); 
          ...
          DB::table('market')
          ->where('entre_id',$id)
          ->update(['beachhead_summary'=>$data->inputdata]);
	}
...
public function sme_update_bus_license(Request $request){
    ...
    public function sme_team_member($id){

      return view('Admin.team.sme_add_team_member',compact('id'));
    } 
  ...
  public function sme_edit_team_member($id){
    ...
    $id = decrypt($id);
     $member=DB::table('team_members')->where('team_id',$id)->get();
     return view('Admin.team.sme_edit_team_member',compact('member'));
    } catch (DecryptException $e) {
        abort(404);
    }   
  }

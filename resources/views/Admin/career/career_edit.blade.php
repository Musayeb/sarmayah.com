...
<section class="blog pt-5 pb-5">
        <div class="container">
            <form method="post" action="{{url('ad/career/update')}}" enctype="multipart/form-data" >
                @csrf
              <div class="row justify-content-center">
                <div class="col-lg-9 col-md-9">
                    <div class="form-group" >
                        <h6 for="blog_title">Job Title</h6>
                        <input type="text" class="form-control " value="{{$blog[0]->blog_title}}"  placeholder="" name="job_title">
                   {!!$errors->first('job_title', '<small class="text-danger ml-2">:message</small>')!!}
                    </div>
                         <div class="form-group ">
                        <h6 for="blog_title">Job Location</h6>
                        <input type="text" class="form-control " value="{{$blog[0]->job_location}}"  placeholder="..." name="job_location">
                   {!!$errors->first('job_location', '<small class="text-danger ml-2">:message</small>')!!}
                    </div>
                         <div class="form-group ">
                        <h6 for="blog_title">Job Catagory</h6>
                        <input type="text" class="form-control " value="{{$blog[0]->job_catagory}}"  placeholder="..." name="job_catagory">
                   ...
                   <div class="form-group ">
                        <h6 for="blog_title">Job Type</h6>
                          <select class="form-control" name="job_type" >
                            ...
                            <option @if($blog[0]->job_type=="Full Time") selected @endif>Full Time</option>
                            <option @if($blog[0]->job_type=="Part Time") selected @endif>Part Time</option>
                   ...
                   {!!$errors->first('job_type', '<small class="text-danger ml-2">:message</small>')!!}
                     ...
                     <div class="form-group ">
                        ...
                        <input type="date" class="form-control " value="{{$blog[0]->job_close_date}}"  placeholder="..." name="job_closing_date">
                   {!!$errors->first('job_closing_date', '<small class="text-danger ml-2">:message</small>')!!}
                    </div>

                    <div class="form-group">
                         <h6 for="blog_content">Job Content</h6>
                        <textarea class="form-control" rows="20" id="mymce1" name="job_content"  placeholder="">{{$blog[0]->blog_content}}</textarea>
                  ...
                  {!!$errors->first('job_content', '<small class="text-danger ml-2">:message</small>')!!}
                    ...
                    <div class="form-group ">
                        <h6 for="blog_title">Job Tags</h6>
                          <input type="text" class="tagator form-control "  name="job_tag"  value="{{$blog[0]->tags}}" style="padding:0xp 0px;display:block">                  
                          {!!$errors->first('job_tag', '<small class="text-danger ml-2">:message</small>')!!}
                    </div>
                    <input type="hidden" name="id" value="{{$blog[0]->blog_id}}">
